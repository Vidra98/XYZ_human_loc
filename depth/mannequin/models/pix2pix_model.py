...
import os
from depth.mannequin.models import base_model
from depth.mannequin.models import networks
import sys
import h5py
import os.path
from skimage.io import imsave
from depth.mannequin.models import hourglass
...
class Pix2PixModel(base_model.BaseModel):
    ...
    def forward(self):
        ...
        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                self.targets['input_log_depth'],
                requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                self.targets['input_confidence'],
                requires_grad=False).unsqueeze(1)
    ...
    def evlaute_M_error(self, input_, targets, n_iter, write_to_summary):
        ...
        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                targets['input_log_depth'], requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                targets['input_confidence'], requires_grad=False).unsqueeze(1)
    ...
    def eval_save_tum_img(self, input_, targets, save_path):
        input_imgs = autograd.Variable(input_)
        human_mask = 1.0 - autograd.Variable(
            targets['env_mask'], requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            targets['keypoints_img'], requires_grad=False).unsqueeze(1)
    ...
    def run_and_save_videos_prediction(self, input_, targets, save_path):
        ...
        mvs_depth = autograd.Variable(targets['mvs_depth'], requires_grad=False)
        input_depth = autograd.Variable(
            targets['input_depth'], requires_grad=False)
    ...
    def run_and_save_VICTOR(self, input, output):
        ...
        out = cv2.VideoWriter('output.mp4',fourcc, 30, (int(resized_width),int(resized_height*2)))
        #Video processing
        ...
        while cap.isOpened():
            ...
            frame = transform.resize(frame, (resized_width,resized_height))
            frame=torch.tensor(frame)
            ...
            #run the model
            start = time.time()
            prediction_d, pred_confidence = self.netG.forward(frame)
            pred_log_d = prediction_d.squeeze(1)
            pred_d = torch.exp(pred_log_d)
            end = time.time()
            ...
            #cv2.imshow('window-name', (saved_img*255).astype(np.uint8))
            ...
            saved_imgs=200*np.ones((resized_width,int(2*resized_height))).astype(np.uint8)
    ...
    def run_and_save_DAVIS(self, input_, targets, save_path):
        ...
        stack_inputs = input_imgs
        print('model input',np.shape(stack_inputs))
